...
class Task_000001_environment extends \canis\setup\Task
{
    ...
    public function run()
    {
        if ($this->fields) {
            ...
            $input = [];
            $input['general'] = [];
            $input['general']['template'] = CANIS_APP_ENVIRONMENT;
            $input['general']['application_name'] = $this->setup->app()->name;

            $input['database'] = [];
            $input['database']['host'] = CANIS_APP_DATABASE_HOST;
            $input['database']['port'] = CANIS_APP_DATABASE_PORT;
            $input['database']['username'] = CANIS_APP_DATABASE_USERNAME;
            $input['database']['password'] = CANIS_APP_DATABASE_PASSWORD;
            $input['database']['dbname'] = CANIS_APP_DATABASE_DBNAME;
            $upgrade = true;
        }
        $input['_'] = [];
    ...
    public function getFields()
    {
        ...
        $fields['general']['fields']['template'] = ['type' => 'select', 'options' => $this->envListOptions, 'label' => 'Environment', 'required' => true, 'value' => function () { return defined('CANIS_APP_ENVIRONMENT') ? CANIS_APP_ENVIRONMENT : 'development'; }];
        ...
        $fields['database']['fields']['host'] = ['type' => 'text', 'label' => 'Host', 'required' => true, 'value' => function () { return defined('CANIS_APP_DATABASE_HOST') ? CANIS_APP_DATABASE_HOST : '127.0.0.1'; }];
        $fields['database']['fields']['port'] = ['type' => 'text', 'label' => 'Port', 'required' => true, 'value' => function () { return defined('CANIS_APP_DATABASE_PORT') ? CANIS_APP_DATABASE_PORT : '3306'; }];
        $fields['database']['fields']['username'] = ['type' => 'text', 'label' => 'Username', 'required' => true, 'value' => function () { return defined('CANIS_APP_DATABASE_USERNAME') ? CANIS_APP_DATABASE_USERNAME : ''; }];
        $fields['database']['fields']['password'] = ['type' => 'text', 'label' => 'Password', 'required' => true, 'value' => function () { return defined('CANIS_APP_DATABASE_PASSWORD') ? '' : ''; }];
        $fields['database']['fields']['dbname'] = ['type' => 'text', 'label' => 'Database Name', 'required' => true, 'value' => function () { return defined('CANIS_APP_DATABASE_DBNAME') ? CANIS_APP_DATABASE_DBNAME : ''; }];
